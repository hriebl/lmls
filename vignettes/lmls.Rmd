...
abstract: |
  ...
  This vignette and the *lmls* package were written for the course "Advanced Statistical Programming with R" at the University of Göttingen and were published on CRAN to provide an accessible introduction to anybody who is curious about location-scale regression.
...
(\#eq:lmls)
\end{equation}
which is a minimal linear model for location and scale (LMLS). The regression coefficients $\gamma_0$ and $\gamma_1$ are the intercept and the slope parameter for the log-standard deviation, and the $\exp$ function is the inverse link (or response) function. It ensures that the predictions for the standard deviation are always valid (= positive). This step is necessary, because the linear predictor $\gamma_0 + x_i \gamma_1$ can become zero or negative for some choices of $\gamma_0$ and $\gamma_1$.
...
The `lmls` package implements two inference algorithms: a frequentist maximum likelihood (ML) algorithm, which it uses by default, and a Bayesian Markov chain Monte Carlo (MCMC) algorithm.
...
where $\psi$ is the digamma function. The term $\psi(1/2) + \log 2$ follows from the relationship between the $\chi^2$ and the gamma distribution and from the logarithmic expectation of the gamma distribution. Rearranging the equation to
...
To run the MCMC sampler on an existing LMLS, we can call the `mcmc()` function on the corresponding object. The sampler requires a model object that contains the design matrices, so we need to make sure the `lmls()` function was called with the argument `light = FALSE`. Finally, we can use the `summary()` function with the argument `type = "mcmc"` to output some summary statistics of the posterior samples.
...
The posterior samples for one regression coefficient, for example for $\gamma_1$, can be extracted and plotted like this:
...
The full conditional of $\betavec$ is given by
...
3. Repeat the following steps *nsim* times:
   1. Sample $\betavec^{(k)}$ from the full conditional in a Gibbs update step.
...
There are a number of R packages with similar capabilities as *lmls*. The popular *mgcv* package, which is typically used to estimate generalized additive models (GAMs), added support for multi-predictor models including the LMLS with a normally distributed response variable in version 1.8. The *gamlss* package implements generalized additive models for location, scale and shape [GAMLSS, @Rigby2005] in a very general way, and the *bamlss* package is a Bayesian alternative to the *gamlss* package.

Compared to these packages, the scope of the *lmls* package is much narrower. It implements only the LMLS with a normally distributed response variable. Other response distributions or the regularization of covariate effects are not supported. Instead, *lmls* aims to be...

- ... **easy to use**. The few exported functions are intuitive and simple.
- ... **stable**. As *lmls* implements only one single, narrow model class, it can use specific and robust inference algorithms.
...
- ... **accessible**. *lmls* was used as a teaching material for a university course. The code is modular and comprehensible, even to R beginners.
...
= \xmat'\qvec,
$$
where $\qvec = [q_i] = (y_i - \mu_i) / \sigma^2_i$ is an $n$-dimensional column vector. The corresponding Fisher information is given by
$$
\I(\betavec)
= \cov(s(\betavec))
= \cov(\xmat'\qvec)
...
= \zmat'\rvec,
$$
where $\rvec = [r_i] = (y_i - \mu_i) / \sigma_i)^2 - 1$ is an $n$-dimensional column vector. The corresponding Fisher information is given by
$$
\I(\gammavec)
= \cov(s(\gammavec))
= \cov(\zmat'\rvec)
